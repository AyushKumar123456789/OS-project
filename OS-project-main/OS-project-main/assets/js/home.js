...
function recalculateServiceTime() {
  ...
  if (algorithm == "fcfs") {
    $.each(inputTable, function (key, value) {
      ...
      //Turn Around time
      $(value.children[6]).text(completionTime - $(value.children[1]).text());
    });
    ...
    for (var i = 0; i < exectuteTimes.length; i++) {
      ...
      $(inputTable[currentIndex + 1].children[6]).text(
        totalExectuteTime - $(inputTable[currentIndex + 1].children[1]).text()
      );
    }
...
function draw() {
  ...
  if (algorithm == "fcfs") {
    ...
    if (quantum <= 0) {
      window.alert("Alright Don't be checky I can't handle negative time");
      return false;
    } else {
      var executeTimes = [];
